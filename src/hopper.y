...
ListaLeia:
	...
	T_Identificador {
		...
		if(strcmp(tipo_string,"string")==0){
			sprintf(s,"%s = window.prompt(\" \")",strdup($1));
		}else{
			sprintf(s,"%s = parse%s(window.prompt(\" \"))",strdup($1),tipo_string);
		}
		tabear();
	...
	| ListaLeia T_Ident_Separador T_Identificador {
		...
		printf(": %s %s %s\n",temp->key.nome, temp->key.escopo, tipo_string);
		if(strcmp(tipo_string,"string")==0){
			sprintf(s,"%s = window.prompt(\" \")",strdup($3));
		}else{
			sprintf(s,"%s = parse%s(window.prompt(\" \"))",strdup($3),tipo_string);
		}
		tabear();
		push_traducao(&fila_traducao,s);
	} 
...
T_Se:
     ...
     T_SE {tabear();push_traducao(&fila_traducao,"if (");} Expr {push_traducao(&fila_traducao, ") ");} 
;
...
T_Senao:
	T_SENAO {tabear_especial();push_traducao(&fila_traducao, "}\n");tabear_especial();push_traducao(&fila_traducao, "else { \n");}
;
...
BlocoPara:
	...
	T_Para T_Identificador{verificar_variavel(strdup($2));sprintf(variavel_escolha, "%s",$2); push_traducao(&fila_traducao,variavel_escolha);} T_De Expr T_Ate {push_traducao(&fila_traducao,variavel_escolha);push_traducao(&fila_traducao,"<=");} T_MeioPara T_Faca  FimComando Comandos T_FimPara

;

T_MeioPara:

	Expr {push_traducao(&fila_traducao,";");push_traducao(&fila_traducao,variavel_escolha);push_traducao(&fila_traducao,"++");}
	|Expr {push_traducao(&fila_traducao,";");push_traducao(&fila_traducao,variavel_escolha);push_traducao(&fila_traducao,"=");push_traducao(&fila_traducao,variavel_escolha);push_traducao(&fila_traducao,"+");} T_Passo Expr
...
BlocoRepita:
	T_REPITA {tabear();push_traducao(&fila_traducao, "do{\n");tab++;} FimComando Comandos T_ATE{tabear_especial();push_traducao(&fila_traducao, "}while ");} Expr {push_traducao(&fila_traducao, ";\n");tab--;}
;
